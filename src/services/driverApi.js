...
const TRACKING_API_BASE_URL =
  import.meta.env.VITE_TRACKING_API_BASE_URL || "/api/tracking";
...
export const driverApi = {
  ...
  async startTracking(token) {
    ...
    // In production: uses absolute URL (https://staff.phuketgevalin.com/api/tracking)
    ...
    const data = await response.json();
    console.log(`API POST ${TRACKING_API_BASE_URL}/start.php:`, {
      status: response.status,
      data,
    });
    return data;
  ...
  async completeTracking(
    ...
    const data = await response.json();
    console.log(`API POST ${TRACKING_API_BASE_URL}/complete.php:`, {
      status: response.status,
      data,
    });
    return data;
